...
4. Add or update the appropriate generated-source category files for part capabilities, render footprints, topology templates, and capability graph entries.
...
12. Regenerate aggregates with the matching `context:*:build` scripts, then run `npm run context:check`, `npm run audit:sources`, `npm run audit:capabilities`, `npm run eval:generalization:report`, and `npm run check`.
